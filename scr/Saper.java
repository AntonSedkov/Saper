...
public class Saper extends JPanel implements AWTEventListener, ActionListener {
	...
	private static int MAX_BOMB_COUNT = 25;
	private int ROWS = 15, COLUMNS = 15, TOTAL = ROWS * COLUMNS;
	...
	private void addControlPanel() {
		...
		bReset.setToolTipText("<HTML>Press <B>F2</B> to reset the current game</HTML>");
	...
	private boolean checkGameState() {
		...
		if (isWin) {
			...
			JOptionPane.showMessageDialog(this, "You win", "Greetings", JOptionPane.INFORMATION_MESSAGE, null);
		}
		return isWin;
	}
	...
	private void blastBombs() {
		...
		lblBombCount.updateUI();
		state = GameState.Finished;
		JOptionPane.showMessageDialog(this, "You lost", "End game", JOptionPane.ERROR_MESSAGE, null);
		for (Component c : pnlMain.getComponents()) {
			GameButton b = (GameButton) c;
			b.setEnabled(false);
		}
	...
	class GameButton extends JButton {
		...
		public String getText() {
			...
			if (state == State.Clicked) {
				if (isBomb) {
					return "<html><font size='17'><b>*</b></font></html>";
				} else {
					if (getBombCount() > 0)
						return getBombCount() + "";
					else
	...
	public void actionPerformed(ActionEvent actionEvent) {
		if (actionEvent.getSource() == bReset) {
			restartGame();
		}
	}
}